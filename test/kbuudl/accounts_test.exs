defmodule Kbuudl.AccountsTest do
  use Kbuudl.DataCase
  ...
  describe "users" do
    ...
    @update_attrs %{name: "some updated name", username: "some updated usrnm"}
    ...
    test "update_user/2 with valid data updates the user" do
      ...
      assert user.username == "some updated usrnm"
    end
  ...
  describe "credentials" do
    alias Kbuudl.Accounts.Credential
    alias Kbuudl.Accounts.User

    @valid_attrs %{name: "some name", username: "some username", credential: %{email: "some email", password: "some password_hash"}}
    @update_attrs %{email: "some updated email", password: "some updated password_hash"}
    @invalid_attrs %{email: nil, password: nil}

    def credential_fixture(attrs \\ %{}) do
      {:ok, %User{credential: credential}} =
        attrs
        |> Enum.into(@valid_attrs)
